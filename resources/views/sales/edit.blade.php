...
<div class="container mx-auto px-4 py-6">
    <div class="flex justify-between items-center mb-6">
        <h1 class="text-2xl font-bold text-gray-800">Edit Sales Record</h1>
    ...
    <div class="bg-white rounded-lg shadow-md p-6">
        <form action="{{ route('sales.data.update', $index) }}" method="POST">
            @csrf
            @method('PUT')
                    ...
                    @if(!in_array($header, ['id', 'created_at', 'updated_at', 'deleted_at']))
                        <div class="mb-4">
                            ...
                            <input type="text" 
                                   ...
                                   value="{{ old($header, $record[$header] ?? '') }}">
                            @error($header)
            ...
            <div class="mt-8 flex justify-between items-center">
                <form action="{{ route('sales.data.destroy', $index) }}" method="POST" onsubmit="return confirm('Are you sure you want to delete this record?')">
                    @csrf
                    @method('DELETE')
